...
class User(Base):
    ...
    user_group_id = Column(String(36), ForeignKey('user_group.id'))
    # dte_sender_id = Column(String(36), ForeignKey('dte_sender.id'))
    ...
    campaigns = relationship('Campaign', backref=backref('campaign_user', uselist=False), lazy=True)
    ulinc_campaigns = relationship('Ulinc_campaign', backref=backref('ulinc_campaign_user', uselist=False), lazy=False)
    contacts = relationship('Contact', backref=backref('contact_user', uselist=False), lazy=False)
    ulinc_cookie = relationship('Ulinc_cookie', backref=backref('ulinc_cookie_user', uselist=False), lazy=True)

class User_type(Base):
    __tablename__ = 'user_type'

    ### Client, User Group Manager, Assistant, DTE_Sender ###

    # Primary Keys
    user_type_id = Column(Integer, primary_key=True, auto_increment=True, nullable=False)
    ...
    user_type_description = Column(String(512), nullable=True)
...
class User_group_manager(Base):
    __tablename__ = 'user_group_manager'

    # Primary Keys
    user_group_manager_id = Column(String(36), primary_key=True, nullable=False)
    ...
    # SQLAlchemy Relationships and Backreferences
    user_groups = relationship('User_group', backref=backref('user_group_manager', uselist=False), lazy=False)

class User_group(Base):
    __tablename__ = 'user_group'

    user_group_id = Column(String(36), primary_key=True, nullable=False)
    ...
    user_group_manager_id = Column(String(36), ForeignKey('user_group_manager.id'), nullable=False)

    users = relationship('User', backref=backref('user_user_group', uselist=False), lazy=False)
...
class Ulinc_campaign(Base):
    ...
    def __init__(self, ulinc_campaign_id, user_id, name, ulinc_is_active, ulinc_campaign_id, ulinc_is_messenger, ulinc_messenger_origin_message):
...
class Contact(Base):
    ...
    def __init__(self, contact_id, user_id, campaign_id, ulinc_campaign_id, webhook_response_id, ulinc_id, ulinc_ulinc_campaign_id, first_name, last_name, title,
...
class Webhook_response(Base):
    ...
    def __init__(self, webhook_response_id, user_id, webhook_response_value, webhook_response_type_id):
        self.webhook_response_id = webhook_response_id
        self.user_id = user_id
...
class Dte(Base):
    ...
    users = relationship('User', backref='dte', lazy=False)
...
class Credentials(Base):
    ...
    # SQLAlchemy Relationships and Backreferences
    users = relationship('User', backref=backref('user_credentials', uselist=True))
...
class Ulinc_user_info(Base):
    __tablename__ = 'ulinc_user_info'

    # Primary Keys
    ulinc_user_info_id = Column(String(36), primary_key=True)
    ...
    # Common Columns
    user_ulinc_id = Column(String(16), nullable=False)
