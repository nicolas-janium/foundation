...
if not os.getenv('LOCAL_DEV'):
    from model import *
    logger = logging.getLogger('get_ulinc_campaigns')
...
def main(request):
    ...
    clients = session.query(Client).filter(Client.client_id == client_id).all()
...
if __name__ == '__main__':
    payload = {
        "client_id": "8a52cdff-6722-4d26-9a6a-55fe952bbef1" # Jonny Karate
    }
    main(payload)
